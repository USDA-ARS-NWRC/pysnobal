...
#     @profile
        ...
        if self.snowcover:
            if self.snow_state.layer_count == 1:
                self.snow_state.T_s_0 = self.new_tsno(
                    self.snow_state.m_s_0, self.snow_state.T_s_0, self.snow_state.cc_s_0)
                ...
                if self.isothermal:
                    ...
                    self.snow_state.T_s_l = self.new_tsno(
                        self.snow_state.m_s_l, self.snow_state.T_s_l, self.snow_state.cc_s_l)
                    self.snow_state.T_s = self.new_tsno(
                        self.snow_state.m_s, self.snow_state.T_s, self.snow_state.cc_s)
    ...
    def h2o_compact(self):
        ...
        if self.precip_now:
            h2o_added = (
                self.snow_state.melt + self.precip_info[self.tstep_level].m_rain) / self.snow_state.m_s
    ...
    def precip_event(self):
        ...
        if self.precip_now:
            if self.snowcover:
                ...
                h2o_vol_snow = self.precip.h2o_sat_snow * self.snow_state.max_h2o_vol
                self.snow_state.h2o += H2O_LEFT(self.precip_info[self.tstep_level].z_snow,
                ...
                self.snow_state.z_s = self.precip_info[self.tstep_level].z_snow
                self.snow_state.rho = self.input1.rho_snow
                self.snow_state.T_s = self.precip.T_snow
                self.snow_state.T_s_0 = self.precip.T_snow
                self.snow_state.T_s_l = self.precip.T_snow
                self.snow_state.h2o_sat = self.precip.h2o_sat_snow
    ...
    def advec(self):
        ...
        if self.precip_now:
            ...
            M = self.heat_stor(CP_WATER(self.precip.T_rain),
                               self.precip_info[self.tstep_level].m_rain,
                               self.precip.T_rain - self.snow_state.T_s_0) + \
                self.heat_stor(CP_ICE(self.precip.T_snow),
                               self.precip_info[self.tstep_level].m_snow,
                               self.precip.T_snow - self.snow_state.T_s_0)
...
#             raise Exception('T_a is below 0 K')
        ...
        # calculate H & L_v_E
#         (rel_z_T, self.z_0, self.input1.T_a, self.snow_state.T_s_0, self.input1.e_a, e_s, self.input1.u, 0.0)
#         (rel_z_T, rel_z_u, self.z_0, self.input1.T_a, self.snow_state.T_s_0, self.input1.e_a, e_s, self.input1.u, 0.0)
        H, L_v_E, E, status = libsnobal.hle1(self.P_a, self.input1.T_a, self.snow_state.T_s_0, rel_z_T,
...
#         return Map({key: 0.0 for key in ['m_pp', 'm_snow', 'm_rain', 'z_snow']})
    ...
    def init_snow(self, from_record=False):
        ...
        if from_record:
            ...
            self.snow_state = SnowState(self.snow_records['z_s'].shape)
        ...
        if self.snow_state.layer_count == 0:
            ...
            self.snow_state.cc_s_0 = self.cold_content(
                self.snow_state.T_s_0, self.snow_state.m_s_0)
