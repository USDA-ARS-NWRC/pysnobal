...
#         print "Current level --> %i" % self.curr_level
        ...
        for i in range(interval):
            ...
            if (self.next_level != SMALL_TSTEP) and (self.below_thold(next_lvl_tstep['threshold'])):
                ...
                self.curr_level = copy(self.next_level)
...
#         if self.precip_now:
        ...
        if self.time_since_out > 0:
            self.em.R_n_bar = TIME_AVG(
                self.em.R_n_bar, self.time_since_out, self.em.R_n, self.time_step)
            self.em.H_bar = TIME_AVG(
                self.em.H_bar, self.time_since_out, self.em.H, self.time_step)
            self.em.L_v_E_bar = TIME_AVG(
                self.em.L_v_E_bar, self.time_since_out, self.em.L_v_E, self.time_step)
            self.em.G_bar = TIME_AVG(
                self.em.G_bar, self.time_since_out, self.em.G, self.time_step)
            self.em.M_bar = TIME_AVG(
                self.em.M_bar, self.time_since_out, self.em.M, self.time_step)
            self.em.delta_Q_bar = TIME_AVG(
                self.em.delta_Q_bar, self.time_since_out, self.em.delta_Q, self.time_step)
            self.em.G_0_bar = TIME_AVG(
                self.em.G_0_bar, self.time_since_out, self.em.G_0, self.time_step)
            self.em.delta_Q_0_bar = TIME_AVG(
                self.em.delta_Q_0_bar, self.time_since_out, self.em.delta_Q_0, self.time_step)
...
#     @profile
    def mass_bal(self):
        ...
        if self.snowcover:
            if self.snow.layer_count == 1:
                self.snow.T_s_0 = self.new_tsno(
                    self.snow.m_s_0, self.snow.T_s_0, self.snow.cc_s_0)
                ...
                if self.isothermal:
                    ...
                    self.snow.T_s_0 = self.new_tsno(
                        self.snow.m_s_0, self.snow.T_s_0, self.snow.cc_s_0)
                    self.snow.T_s_l = self.new_tsno(
                        self.snow.m_s_l, self.snow.T_s_l, self.snow.cc_s_l)
                    self.snow.T_s = self.new_tsno(
                        self.snow.m_s, self.snow.T_s, self.snow.cc_s)
    ...
    def runoff(self):
        ...
        self.snow.h2o_max = H2O_LEFT(
            self.snow.z_s, rho_dry, self.snow.max_h2o_vol)
    ...
    def h2o_compact(self):
        ...
        if self.precip_now:
            h2o_added = (
                self.em.melt + self.precip_info[self.tstep_level].m_rain) / self.snow.m_s
    ...
    def evap_cond(self):
        ...
        if self.snow.layer_count > 0:
            self.adj_snow(
                ((self.em.E_s + (prev_h2o_tot - self.snow.h2o_total)) / self.snow.rho) / 2.0, self.em.E_s)
    ...
    def snowmelt(self):
        ...
        if self.snow.layer_count == 2:
            Q_l = ((self.em.G - self.em.G_0) *
                   self.time_step) + self.snow.cc_s_l
        ...
        if self.snow.cc_s_0 < 0:
            ...
            if self.snow.h2o_total > 0:
                Q_freeze = self.snow.h2o_total * \
                    (self.snow.z_s_0/self.snow.z_s) * libsnobal.LH_FUS(FREEZE)
                Q_left = Q_0 + Q_freeze

                if Q_left <= 0:
                    h2o_refrozen = self.snow.h2o_total * \
                        (self.snow.z_s_0/self.snow.z_s)
                    self.snow.cc_s_0 = Q_left
                else:
                    h2o_refrozen = self.snow.h2o_total * \
                        (self.snow.z_s_0/self.snow.z_s) - MELT(Q_left)
                    self.snow.cc_s_0 = 0
        ...
        if (self.snow.layer_count == 2) and (self.snow.cc_s_l < 0.0):
            ...
            if self.snow.h2o_total > 0.0:
                Q_freeze = self.snow.h2o_total * \
                    (self.snow.z_s_l/self.snow.z_s) * libsnobal.LH_FUS(FREEZE)
                Q_left = Q_l + Q_freeze

                if Q_left <= 0.0:
                    h2o_refrozen += self.snow.h2o_total * \
                        (self.snow.z_s_l/self.snow.z_s)
                    self.snow.cc_s_l = Q_left
                else:
                    h2o_refrozen += ((self.snow.h2o_total *
                                      (self.snow.z_s_l/self.snow.z_s)) - MELT(Q_left))
    ...
    def precip_event(self):
        ...
        if self.precip_now:
            if self.snowcover:
                ...
                self.adj_snow(
                    self.precip_info[self.tstep_level].z_snow, self.precip_info[self.tstep_level].m_pp)
...
#     @profile
    ...
    def adj_layers(self):
        ...
        if self.snow.layer_count == 0:
            ...
            index = ['h2o', 'h2o_max', 'h2o_total',
                     'h2o_vol', 'm_s', 'cc_s', 'm_s_0', 'cc_s_0']
            for i in index:
                self.snow[i] = 0
            ...
            if (prev_layer_count == 1) and (self.snow.layer_count == 2):
                ...
                self.snow.cc_s_l = self.cold_content(
                    self.snow.T_s_l, self.snow.m_s_l)
...
#     @profile
    ...
    def e_bal(self):
        ...
        if self.snow.layer_count > 0:
            ...
            # temperature.
            # replaces _net_rad()
            self.em.R_n = self.input1.S_n + \
                (SNOW_EMISSIVITY * (self.input1.I_lw -
                                    STEF_BOLTZ * np.power(self.snow.T_s_0, 4)))
            ...
            self.em.delta_Q_0 = self.em.R_n + self.em.H + \
                self.em.L_v_E + self.em.G_0 + self.em.M
    ...
    def g_snow(self):
        ...
        if self.snow.T_s_0 == self.snow.T_s_l:
            ...
            g = libsnobal.ssxfr(k_s1, k_s2, self.snow.T_s_0,
                                self.snow.T_s_l, self.snow.z_s_0, self.snow.z_s_l)
...
#         return pd.Series(index=['m_pp','m_snow','m_rain','z_snow'])
    ...
    def init_snow(self, from_record=False):
        ...
        if from_record:
            ...
            self.snow = {key: np.zeros_like(self.snow_records['z_s']) for key in cols}
        ...
        if self.snow.layer_count == 0:
            ...
            self.layer_mass()
            self.snow.cc_s_0 = self.cold_content(
                self.snow.T_s_0, self.snow.m_s_0)
